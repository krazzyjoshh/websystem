...
export default function AdminSellers({ sellers, inviteCodes }) {
    const generateCode = () => {
        router.post('/admin/sellers/invite', { note: 'Auto-generated for new seller' });
    };
    return (
        <AdminLayout title="Sellers Management">
                        ...
                        {seller.seller_profile && (
                            <>
                                ...
                                <div style={{ display: 'flex', gap: 12, marginTop: 12, alignItems: 'center' }}>
                                    ...
                                    {seller.seller_profile.is_verified ? (
                                        <span className="badge badge-cyan"><CheckCircle size={12} style={{ marginRight: 4 }} />Verified</span>
                                    ) : (
                                        <button className="badge badge-warning" onClick={() => router.put(`/admin/sellers/${seller.id}/verify`)} style={{ cursor: 'pointer', border: 'none' }}>Verify Seller</button>
                                    )}
                                </div>
                            </>
                        )}
            ...
            <div style={{ marginTop: 48 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                    <h2 style={{ fontFamily: 'Space Grotesk', fontSize: 20, fontWeight: 700 }}>Invite Codes</h2>
                    ...
                    <table className="admin-table">
                        ...
                        <tbody>
                            {inviteCodes && inviteCodes.map((invite) => (
                                <tr key={invite.id}>
                                    <td style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: 16 }}>{invite.code}</td>
                                    <td>{invite.creator?.name}</td>
                                    <td>
                                        {invite.is_used ? (
                                            <span className="badge badge-error">Used</span>
                                        ) : (
                                            <span className="badge badge-success">Available</span>
                                        )}
                                    </td>
                                    <td>{invite.user ? invite.user.name : '-'}</td>
                                    <td>{new Date(invite.created_at).toLocaleDateString()}</td>
                                </tr>
                            ))}
                            {(!inviteCodes || inviteCodes.length === 0) && (
                                <tr>
                                    <td colSpan="5" style={{ textAlign: 'center', color: '#666' }}>No invite codes generated yet.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
